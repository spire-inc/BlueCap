    ...
    internal func didDiscoverCharacteristics(_ characteristics: [CBCharacteristicInjectable], error: Swift.Error?) {
        ...
        if let error = error {
            ...
            characteristics.forEach { cbCharacteristic in
                ...
                Logger.debug("Discovered characteristicc uuid=\(cbCharacteristic.uuid.uuidString), characteristic name=\(bcCharacteristic.name), service name \(name), service uuid \(uuid)")
